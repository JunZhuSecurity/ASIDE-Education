...
public class AsideMarkerSpecialOutputResolutionGenerator implements
	 ...
	 */
	public AsideMarkerSpecialOutputResolutionGenerator() {
		super();
		ranGen = new Random();
	...
	private IMarkerResolution[] internalGetResolutions(IMarker marker) {
		...
		logger.info(dateFormat.format(hoverDate) + " " + AsidePlugin.getUserId()
				+ " hovers over the marker or clicks on the marker of input validation warning at Line "
		...
		// change definedInputTypeList to tmpList, but will be assigned to
	...
	private boolean internalHasResolutions(IMarker marker) {
		...
		try {
			...
			for (int i = 0; i < result.length; i++) {
				if (result[i].equals("specialOutput")) {
...
//		logger.info(dateFormat.format(showGuidanceDate)
//				+ " Guidance view displays corresponding detail information for the selected marker at "
//				+ targetMarker.getAttribute(IMarker.LINE_NUMBER, -1)
//				+ " in File <<" + fCompilationUnit.getElementName()
...
//			System.err
//					.println("not right in AsideMarkerSpecialOutputResolutionGenerator");
