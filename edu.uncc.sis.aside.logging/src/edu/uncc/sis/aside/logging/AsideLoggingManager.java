...
public class AsideLoggingManager {
	private ILog log;
	 ...
	 * the lits of acctive plug-in log managers.
	 * 
	 * @param plugin
	 *            the plug-in object
	 * @param properties
	 ...
	 */
	public AsideLoggingManager(Plugin plugin, Properties properties) {
		this.log = plugin.getLog();
		this.stateLocation = plugin.getStateLocation();
		System.out.println("this.stateLocation = " + this.stateLocation.toString());
		
		hierarchy = new Hierarchy(new RootLogger(Level.DEBUG));
		hierarchy.addHierarchyEventListener(new AsideEventListener());
		new PropertyConfigurator().doConfigure(properties, hierarchy);
		AsideLoggingPlugin.getDefault().addLogManager(this);
	}

	/**
	 * Hooks a plug-in into this PluginLogManager. When the hooked plug-in uses
	 * the Eclipse log API, it will be channeled to this logging framework:
	 * Eclipse native log
	 * 
	 * @param id
	 *            logger name (usually the the plug-in id)
	 * @param log
	 *            plug-in log
	 */
	public boolean hookPlugin(String id, ILog log) {
		synchronized (this.hookedPlugins) {
			...
			AsideLoggingListener listener = new AsideLoggingListener(log,
					getLogger(id));
	...
	/**
	 * Unhooks a plug-in from this PluginLogManager. The Eclipse log API won't
	 * be channeled to this logging framework anymore.
	 * 
	 * @param id
	 *            logger name (usually the the plug-in id)
	 */
	public boolean unHookPlugin(String id) {
		synchronized (this.hookedPlugins) {
			...
			AsideLoggingListener listener = (AsideLoggingListener) this.hookedPlugins
					.get(id);
			listener.dispose();
	...
	public void shutdown() {
		internalShutdown();
		AsideLoggingPlugin.getDefault().removeLogManager(this);
	}

	/**
	 * Used by LoggingPlugin to shutdown without removing it from the
	 * LoggingPlugin list
	 * 
	 */
	...
	class AsideEventListener implements HierarchyEventListener {

		/**
		 * Called when a new appender is added for a particular level.
		 * Internally it checks if the appender is one of our custom ones and
		 * sets its custom properties.
		 * 
		 * @param category
		 *            level
		 * @param appender
		 *            appender added for this level
		 */
		public void addAppenderEvent(Category cat, Appender appender) {
			if (appender instanceof EclipseLogAppender) {
				((EclipseLogAppender) appender).setLog(log);
			}
			if (appender instanceof AsideFileAppender) {
				((AsideFileAppender) appender).setStateLocation(stateLocation);
			}
		}

		/**
		 * Called when a appender is removed from for a particular level. Does
		 * nothing.
		 * 
		 * @param category
		 *            level
